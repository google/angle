...
class EmulateMutableFunctionParamsTraverser : public TLValueTrackingTraverser
{
  public:
    ...
    void visitSymbol(TIntermSymbol *node) override
    {
        ...
        mParamUsages[param].push_back({node, getParentNode()});
    }

    bool update(TCompiler *compiler, TIntermBlock *root)
    {
        for (const Param &param : mParamsWrittenTo)
        {
            // Declare the temporary and initialize it with the parameter.
            TIntermDeclaration *tempVarDecl = nullptr;
            TVariable *tempVar = DeclareTempVariable(mSymbolTable, new TIntermSymbol(param.second),
                                                     EvqTemporary, &tempVarDecl);

            // Put the declaration at the top of the function body.
            insertStatementsInBlockAtPosition(param.first->getBody(), 0, {tempVarDecl}, {});

            // Replace all the references to the parameter with references to the temp var.
            for (const ParamUsageWithParent &paramUse : mParamUsages[param])
            {
                queueReplacementWithParent(paramUse.second, paramUse.first,
  ...
  private:
    using Param                = std::pair<const TIntermFunctionDefinition *, const TVariable *>;
    using ParamUsageWithParent = std::pair<TIntermSymbol *, TIntermNode *>;
    TMap<Param, TVector<ParamUsageWithParent>> mParamUsages;
    TSet<Param> mParamsWrittenTo;
