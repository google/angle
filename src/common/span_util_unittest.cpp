...
TEST(SpanMemcpy, EmptyCopyWithin)
{
    std::vector<char> src(2, 'A');
    std::vector<char> dst(4, 'B');
    angle::SpanMemcpy(angle::Span(dst).subspan<1u>(), angle::Span(src).subspan<2u>());
    EXPECT_EQ(dst[0], 'B');
    EXPECT_EQ(dst[1], 'B');
    EXPECT_EQ(dst[2], 'B');
    EXPECT_EQ(dst[3], 'B');
...
TEST(SpanMemcpy, EmptyCopyToEmpty)
{
    std::vector<char> src(2, 'A');
    std::vector<char> dst(4, 'B');
    angle::SpanMemcpy(angle::Span(dst).subspan<4u>(), angle::Span(src).subspan<2u>());
    EXPECT_EQ(dst[0], 'B');
    EXPECT_EQ(dst[1], 'B');
    EXPECT_EQ(dst[2], 'B');
    EXPECT_EQ(dst[3], 'B');
...
// Test that spanse() writes all the bytes of a larger type.
