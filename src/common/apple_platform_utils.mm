...
{
    static bool queriedSystemDevices  = false;
    static bool gpuFamiliesSufficient = false;
...
#endif
    {
        if (!queriedSystemDevices)
        {
            ANGLE_APPLE_OBJC_SCOPE
            {
                queriedSystemDevices = true;
                auto devices         = [MTLCopyAllDevices() ANGLE_APPLE_AUTORELEASE];
                if (!devices || ([devices count] == 0))
                {
                    return false;
                }

                bool familiesSufficient = true;
                ...
                gpuFamiliesSufficient = familiesSufficient;
            }
        }

        return gpuFamiliesSufficient;
    }
    return false;
}
