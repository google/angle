...
{
    vk::DescriptorPoolPointer pool;

    ANGLE_TRY(descriptorPool.allocateDescriptorSet(contextVk, descriptorSetLayout, &pool,
                                                   descriptorSetOut));
    ...
    commandBufferHelper->retainResource(pool.get());
