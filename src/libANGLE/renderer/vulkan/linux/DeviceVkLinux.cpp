...
egl::Error DeviceVkLinux::initialize()
{
    ...
    {
        char deviceName[50];
        snprintf(deviceName, sizeof(deviceName), "/dev/dri/card%ld", drmProperties.primaryMinor);
    ...
    {
        char deviceName[50];
        snprintf(deviceName, sizeof(deviceName), "/dev/dri/renderD%ld", drmProperties.renderMinor);
