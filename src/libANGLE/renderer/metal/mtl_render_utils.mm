...
{
    ObjCPtr<id<MTLFunction>> &shader = *shaderOut;
    if (shader)
    {
        return angle::Result::Continue;
    }
...
angle::Result ClearUtils::ensureShadersInitialized(ContextMtl *ctx, uint32_t numOutputs)
{
    ANGLE_MTL_OBJC_SCOPE
    {
        ...
        if (!mFragmentShaders[numOutputs])
        {
            NSError *err             = nil;
            id<MTLLibrary> shaderLib = ctx->getDisplay()->getDefaultShadersLib();
            ObjCPtr<MTLFunctionConstantValues> funcConstants =
                                             ...
                                             const ClearRectParams &params)
{
    ...
    ObjCPtr<id<MTLRenderPipelineState>> renderPipelineState;
...
angle::Result ColorBlitUtils::ensureShadersInitialized(ContextMtl *ctx,
                                                       const ShaderKey &key,
    ...
    ObjCPtr<id<MTLRenderPipelineState>> *outPipelineState)
{
    ...
    ObjCPtr<id<MTLFunction>> *fragmentShader = &mBlitFragmentShaders[key];
    ...
    ObjCPtr<id<MTLComputePipelineState>> *outComputePipelineState)
{
    ...
    ObjCPtr<id<MTLFunction>> &shader = mStencilBlitToBufferComputeShaders[sourceStencilTextureType];
    if (!shader)
    {
        ANGLE_MTL_OBJC_SCOPE
        {
    ...
    ObjCPtr<id<MTLRenderPipelineState>> *outRenderPipelineState)
{
    ...
    ObjCPtr<id<MTLFunction>> *fragmentShader     = nullptr;
    ...
    const StencilBlitViaBufferParams &params)
{
    ...
    ObjCPtr<id<MTLComputePipelineState>> pipeline;
    ...
    ObjCPtr<id<MTLComputePipelineState>> *outComputePipeline)
{
    ...
    ObjCPtr<id<MTLFunction>> &shader     = mIndexConversionShaders[srcTypeKey][aligned ? 1 : 0];
    ...
    ObjCPtr<id<MTLComputePipelineState>> *outComputePipeline)
{
    ...
    ObjCPtr<id<MTLFunction>> &shader = (*shaderArray)[srcTypeKey][aligned ? 1 : 0];
                                                         ...
                                                         const IndexConversionParams &params)
{
    ...
    ObjCPtr<id<MTLComputePipelineState>> pipelineState;
    ANGLE_TRY(
    ...
    ObjCPtr<id<MTLComputePipelineState>> *outComputePipeline)
{
    ...
    ObjCPtr<id<MTLFunction>> &shader     = mVisibilityResultCombineComputeShaders[keepOldValueVal];
    if (!shader)
    {
        ANGLE_MTL_OBJC_SCOPE
        {
    ...
    const VertexFormatConvertParams &params)
{
    ...
    ObjCPtr<id<MTLRenderPipelineState>> pipeline;
