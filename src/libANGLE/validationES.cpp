                            ...
                            const void *pixels)
{
    ...
    if (isRobust && (width > 0 && height > 0))
    {
        auto getClippedExtent = [](GLint start, GLsizei length, int bufferSize) {
            ASSERT(length >= 0);
            ASSERT(bufferSize >= 0);
            ASSERT(angle::base::CheckAdd<GLint>(start, length).IsValid());
            // This expression cannot overflow with the assertions above.
            return std::max<int>(0, std::min<int>(start + length, bufferSize) - std::max(0, start));
        };
        ...
        Extents readBufferSize = readBuffer->getSize();
        const int columnsValue = getClippedExtent(x, width, readBufferSize.width);
        const int rowsValue    = getClippedExtent(y, height, readBufferSize.height);
